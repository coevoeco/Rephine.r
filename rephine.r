...
hmmmerge<-function(panpath,flavor,bit,threads)
{
	...
	if(flavor==0)														#for flavor 0, need the list of gcs with a fusion pre-HMM merging
	{
	...
	for(i in 1:length(hmmcomp$csize))									#iterate through every hmm connected component
	{
		...
		if(flavor==0)													#in flavor 0, update the gcs in each merger with paths to the fused versions when needed
		{
		...
		newfile=character()
		for(j in 1:length(templist))									#iterate through each cluster and write to a new file.
		{
			newfile=c(newfile,readLines(templist[j]))					
		}
	...
	if(flavor==0)													#the unchanged list also has to be udpated (including path) for flavor 0 to account for possible fusions.
	{
...
fragfuse<-function(panpath,contigspath,flavor,relbit,maxpercoverlap,threads)
{
	...
	clustdir="clusters_merged"								#By default, assume HMM merging was run but switch to original clusters if no HMM merge results are present.
	...
	for(i in 2:length(filelist)){fragtable=rbind(fragtable,read.table(filelist[i]))}	#pool each fragtable into one big matrix.
	...
	for(i in 1:length(fraggenomes))									#iterate through each genome
	{
		...
		for(j in 1:length(tempfrags))								#iterate through each potential fragment and record information about the start, stop, and read direction
		{
			...
			if(fusecheck1==0)
			{
				pos1=which(as.numeric(tempcalls[,1])==as.numeric(fragtable[tempfrags[j],'gid1']))	#when running a second round of fusions, gid's might contain ":" and won't match any original gene call. Need an if statement to correct this error
	...
	fragtable=cbind(fragtable,maxrelblast)														
	fragtable=cbind(fragtable,ngenecol)															
	colnames(fragtable)[16]='ngenes'
	
	if(rerun==0){write.table(fragtable,'../allgcfrags_table_wmergers.txt',row.names=F,quote=F,sep='\t')}	
	...
	fragpos=which(as.numeric(fragtable[,'percoverlap']) < maxpercoverlap & as.numeric(fragtable[,'maxrelblast']) < relbit)			
	if(length(fragpos)==0)
	{
...
hmmprep<-function(panpath,threads)			
{
		...
		system('mkdir HMMs')
		setwd('clusters_aligned')
		if(threads==1){system('for file in $(ls); do hmmbuild ../HMMs/${file%.fa}.hmm $file; done')}		
